...
namespace Web.ITroc.Persistence.Repositories
{
	...
	public class ApiCollectionRepository : IApiCollectionRepository
	{
		...
		public async Task<IEnumerable<AdsDto>> GetOneAdFromDb(int id)
		{
			...
			{
				Id = x.Id,
				AdCreate = x.AdCreate.ToString("D"),
				AdTitle = x.AdTitle,
		...
		public async Task<Ads> GetAdFromDbByCurentUser(int id, string userId)
		{
			var result = await _context.Adses
				.SingleOrDefaultAsync(x => x.Id == id && x.Poubelle == false && x.UserId == userId);
			if (result != null)
				_context.Adses.Remove(result);
